...
jQuery && function (t) {
	function i(i, n) {
		...
		s(), r || d.hasClass('spid-idp-button-disabled') || (
			d.addClass('spid-idp-button-open'), o.data('spid-idp-button-trigger', d).show(), e(), o.trigger('show',
				{spidIDPButton: o, trigger: d})
		);
	}
	...
	function e() {
		var i = t('.spid-idp-button:visible').eq(0), s = i.data('spid-idp-button-trigger'),
			e = s ? parseInt(s.attr('data-horizontal-offset') || 0, 10) : null,
			n = s ? parseInt(s.attr('data-vertical-offset') || 0, 10) : null;
		0 !== i.length && s && (
			i.hasClass('spid-idp-button-relative') ? i.css({
				left: i.hasClass('spid-idp-button-anchor-right') ? s.position().left - (
				                                                   i.outerWidth(!0) - s.outerWidth(!0)
					) - parseInt(s.css('margin-right'), 10) + e : s.position().left + parseInt(
						s.css('margin-left'), 10) + e,
				top: s.position().top + s.outerHeight(!0) - parseInt(s.css('margin-top'), 10) + n,
			}) : i.css({
				left: i.hasClass('spid-idp-button-anchor-right') ? s.offset().left - (
				                                                   i.outerWidth() - s.outerWidth()
					) + e : s.offset().left + e, top: s.offset().top + s.outerHeight() + n,
			})
		);
	}
	...
	}), t(document).on('click.spid-idp-button', '[spid-idp-button]', i), t(document).on('click.spid-idp-button',
		s), t(window).on('resize', e);
}(jQuery);
